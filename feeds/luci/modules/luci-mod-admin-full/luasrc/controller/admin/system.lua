...
function index()
	...
	entry({"admin", "system"}, alias("admin", "system", "system"), _("System"), 24).index = true
...
function action_backup()
	...
	local revision
	local file = io.open("/etc/revision", "r")
	if file ~= nil then
		revision = file:read("*line")
	...
	luci.http.header(
		'Content-Disposition', 'attachment; filename="backup-%s-%s-%s.tar.gz"' %{
			luci.sys.hostname(),
			os.date("%Y-%m-%d"),
			revision
		})
